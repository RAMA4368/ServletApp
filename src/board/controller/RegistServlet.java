/*
 * jsp만으로 개발했떤 방식에서 regist.jsp 가 담당하는 업무를 
 * 서블릿으로 구현해본다!!!
 * */
...
public class RegistServlet extends HttpServlet{
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		ServletContext context=request.getServletContext();//어플리케이션과 관련된 정보를 담는 객체인 SerlvetContext를 얻자
		//SerlvetContext는 jsp에서는 application내장객체로 접근해야 한다..
		...
		try {
			...
			for( FileItem item : items) {
				if(item.isFormField()) {//일반텍스트 컴포넌트라면.. 작성자,제목,내용등.. textfield 
					...
					board.setFilename(newName);//새로만들어진 이름을 vo담아야 insert 문에 들어감
					//파일 물리적으로 저장
